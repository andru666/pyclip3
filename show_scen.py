...
class Scenarii(App):
    ...
    def show_scen(self, msg):
        ...
        for line in self.DOMTree.toprettyxml().splitlines():
            if 'value' in line:
                layout_popup.add_widget(MyLabel(text='', height=fs/10, bgcolor=(1, 1, 0, 0.3)))
                ...
                if ma:
                    ...
                    lab = MyLabel(text="%-20s : %s" % ((p_name), (p_value)), halign= 'left')
                    layout_popup.add_widget(lab)
                else:
                    lab = MyLabel(text=str(line.strip()), halign= 'left')
                    layout_popup.add_widget(lab)
