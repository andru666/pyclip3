...
def my_excepthook(excType, excValue, tb):
    ...
    with open(os.path.join(mod_globals.crash_dir, 'crash_'+str(time.strftime("%Y-%m-%d-%H.%M.%S", time.localtime()))+'.txt'), 'w') as fout:fout.write(str(string))
...
class screenConfig(App):
    ...
    def make_box_switch(self, str1, active, callback = None):
        ...
        glay = MyGridLayout(cols=2, height=fs*4, size_hint=(1, None), padding=fs/3)
    ...
    def make_opt_ecuid(self, callback = None):
        ...
        if mod_globals.opt_ecu:
            ...
            iText = ''
        ti = TextInput(text=iText, multiline=False, size_hint_x=1.4)
        ...
        if callback:
            sw.bind(active=callback)
        self.button[str1] = sw
        glay = MyGridLayout(cols=3, height=fs*4, size_hint=(1, None), padding=fs/3)
        glay.add_widget(label1)
        glay.add_widget(sw)
        glay.add_widget(ti)
        return glay
    ...
    def make_language_entry(self):
        ...
        self.lang_dropdown = DropDown(height=fs)
        label1.bind(size=label1.setter('text_size'))
        glay = MyGridLayout(cols=2, height=fs*4, size_hint=(1, None), padding=fs/3)
        ...
        if mod_globals.opt_lang:
            ...
            txt = 'SELECT'
        self.langbutton = MyButton(text=txt, height=fs*3)
    ...
    def build(self):
        ...
        gobtn = MyButton(text='START', font_size=fs*3, size_hint=(1, None), on_press=self.finish)
...
def main():
    ...
    if mod_globals.opt_demo and len(mod_globals.opt_ecuid) > 0 and mod_globals.opt_ecuid_on:
        ...
        se.scanAllEcus()
    lbltxt = MyLabel(text='Loading language', font_size=20)
    ...
    while 1:
        ...
        if mod_globals.opt_demo:
            lbltxt = MyLabel(text='Loading dump', font_size=20)
            ...
            lbltxt = MyLabel(text='Save dump', font_size=20)
            popup_init = Popup(title='Initializing', content=lbltxt, size=(400, 400), size_hint=(None, None))
            popup_init.open()
            base.EventLoop.idle()
            sys.stdout.flush()
